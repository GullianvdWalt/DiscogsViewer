...
@PropertySource("classpath:api.properties") // API keys are hidden in api properties file.
...
//            submissions = map.get
    ...
    public String getRelease(JpaOAuthConsumerToken accessToken) {
        ...
        String json = "";
        try {
            InputStream inputStream = consumerSupport.readProtectedResource(new URL(DISCOGS_GET_TEST_RELEASE_URL),
                    accessToken.toOAuthConsumerToken(), "GET");
            Scanner s = new Scanner(inputStream).useDelimiter("\\A");
            json = s.hasNext() ? s.next() : "";
            System.out.println(json);
